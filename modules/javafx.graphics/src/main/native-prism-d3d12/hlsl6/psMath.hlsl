...
void computeLight(float i, float3 normal, float3 refl, float specPower, float3 toLight, float3 lightDir, in out float3 diff, in out float3 spec) {
    ...
    if (gLight[i].attenuation.w < 0.5) {
        diff += saturate(dot(normal, -lightDir)) * gLight[i].color.rgb;
        spec += pow(saturate(dot(-refl, -lightDir)), specPower) * gLight[i].color.rgb;
        return;
    }

    float dist = length(toLight);
    if (dist > gLight[i].range.x) {
        return;
    }
    float3 l = normalize(toLight);

    float cosOuter = gLight[i].spotLightFactors.x;
    float denom = gLight[i].spotLightFactors.y;
    float falloff = gLight[i].spotLightFactors.z;
    float spotlightFactor = computeSpotlightFactor3(l, lightDir, cosOuter, denom, falloff);

    float ca = gLight[i].attenuation.x;
    float la = gLight[i].attenuation.y;
    float qa = gLight[i].attenuation.z;
