...
void transformVertexAttributes(float4 modelVertexPos, float4 modelVertexNormal, in out VsOutput vsOutput) {
    float4 worldVertexPos = mul(modelVertexPos, gData.worldMat);

//  needed for pixel lighting
//  vsOutput.worldPos = worldVertexPos;

    vsOutput.projPos = mul(worldVertexPos, gData.viewProjMat);

    float3 n[3];
    quatToMatrix(modelVertexNormal, n);
    for (int i = 0; i != 3; ++i) {
        n[i] = mul(n[i], (float3x3) gData.worldMat);
    }
...
//  r.worldNormals = n;
    ...
    float3 worldVecToEye = (gData.cameraPos - worldVertexPos).xyz;
    vsOutput.worldVecToEye = getLocalVector(worldVecToEye, n);

    for (int k = 0; k < numLights; ++k) {
        float3 worldVecToLight = gLight[k].pos.xyz - worldVertexPos.xyz;
        vsOutput.worldVecsToLights[k] = getLocalVector(worldVecToLight, n);
        float3 worldNormLightDir = gLight[k].normDir.xyz;
