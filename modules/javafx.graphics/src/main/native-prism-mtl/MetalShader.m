...
- (id) initWithContext:(MetalContext*)ctx withFragFunc:(NSString*) fragName
{
    ...
    if (self) {
        context = ctx;
        SHADER_LOG(@"MetalShader.initWithContext()----> fragName: %@", fragName);
...
JNIEXPORT jlong JNICALL Java_com_sun_prism_mtl_MTLShader_nDisable
  (JNIEnv *env, jclass jClass, jlong shader)
{
    SHADER_LOG(@"-> Native: MTLShader_nDisable");
    MetalShader *mtlShader = (MetalShader *)jlong_to_ptr(shader);
    //[mtlShader disable];
    return 1;
}
...
{
    SHADER_LOG(@"-> Native: MTLShader_nSetFloat");
    MetalShader *mtlShader = (MetalShader *)jlong_to_ptr(shader);
...
{
    SHADER_LOG(@"-> Native: MTLShader_nSetFloat2");
    MetalShader *mtlShader = (MetalShader *)jlong_to_ptr(shader);
    NSString *nameString = jStringToNSString(env, name);
...
{
    SHADER_LOG(@"-> Native: MTLShader_nSetFloat3");
    MetalShader *mtlShader = (MetalShader *)jlong_to_ptr(shader);
    NSString *nameString = jStringToNSString(env, name);
...
{
    SHADER_LOG(@"-> Native: MTLShader_nSetFloat4");
    MetalShader *mtlShader = (MetalShader *)jlong_to_ptr(shader);
    NSString *nameString = jStringToNSString(env, name);
